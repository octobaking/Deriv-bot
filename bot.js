...
// ===================== NEW DERIV API AUTHENTICATION (OTP-BASED) =====================
            ...
            res.on('end', () => {
                try {
                    ...
                    if (response.data && response.data.length > 0) {
                        resolve(response.data[0].account_id);
                    } else {
...
async function getOTP(id) {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            hostname: 'api.derivws.com',
            path: `/trading/v1/options/accounts/${id}/otp`,
            method: 'POST',
            headers: {
                'Deriv-App-ID': CONFIG.APP_ID,
                'Authorization': `Bearer ${CONFIG.API_TOKEN}`,
            ...
            res.on('end', () => {
                try {
                    ...
                    if (response.data && response.data.url) {
                        ...
                        reject('No WebSocket URL in response');
                    }
                } catch (e) {
                    reject(`Parse error: ${e.message}`);
                }
...
async function connectWebSocket(){
    try {
        ...
        log('✅ Got authenticated WebSocket URL');
        
        log('🔗 Connecting to WebSocket...');
        ws = new WebSocket(wsUrl);
        ws.on('open',()=>{
            log('✅ WebSocket AUTHENTICATED and connected');
            subscribeAll();
            setInterval(()=>{ if(ws.readyState===WebSocket.OPEN) ws.ping(); }, 30000);
        });
        ws.on('message', handleMessage);
        ws.on('error', err=>log(`❌ WS error: ${err.message}`));
        ws.on('close',()=>{ log('⚠️ WS closed, reconnecting...'); setTimeout(connectWebSocket, 5000); });
    } catch (error) {
